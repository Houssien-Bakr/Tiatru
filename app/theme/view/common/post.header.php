<?php if ($Listing['private'] == '1' and !$AuthUser['id']) { ?>
    <div class="embed-lock">
        ...
        <div class="subtext"><?php echo __('This content is only for members.'); ?>
            <a href="<?php echo APP . '/login'; ?>" class="text-white font-weight-bold"><?php echo __('Login'); ?></a>,
            <a href="<?php echo APP . '/register'; ?>" class="text-white font-weight-bold"><?php echo __('Register'); ?></a>
        </div>
    </div>
<?php } else { ?>
    ...
    <div class="detail-header d-flex align-items-center">
                ...
                foreach ($Languages as $Language) {
                    ...
                    <?php if ($i == 1) { ?>
                        <?php if (get($Settings, 'data.googleadsense', 'general') == 1) {
                            ...
                            <a class="dropdown-toggle btn-service selected" href="#" data-embed="<?php echo $Language['id'] ?>" <?php if (count($Languages) > 1) { ?> role="button" id="videoSource" data-toggle="dropdown" aria-haspopup="true"
                                    aria-expanded="false" <?php } ?>>
                                <?php echo 'سيرڤرات المشاهدة:'; ?> <span
                                    style="margin-left:0px;"><?php echo ($Language['name'] ? $Language['name'] : $Language['service_name']); ?></span>
                            </a>
                        <?php } ?>
                    <?php } ?>
                    <?php if (count($Languages) > 1 and $i == 1) { ?>
                        <div class="dropdown-menu" aria-labelledby="videoSource"
                            style="position: absolute;z-index:999999;height:95px;background-color: transparent;">
                        <?php } ?>
                        <?php if (count($Languages) > 1) { ?>
                            <?php echo '<button type="button" class="btn-service dropdown-source';
                            if ($i == 1)
                                echo ' selected';
                            echo '" data-embed="' . $Language['id'] . '" style="display:block!important;background-color:#222 !important;line-height: 40px;margin-bottom:7px;">
                                <span class="name" style="float:left;height: 40px !important;font-size: 12px !important;color: var(--theme-color);">' . ($Language['name'] ? $Language['name'] : $Language['service_name']) . '</span>
                                <span style="margin-left:0px;">' . $Language['language_name'] . '</span>
                            </button>'; ?>
                        <?php } ?>
        ...
        <?php } ?>
        <?php if ($AuthUser['id']) { ?>
            <button type="button" class="btn btn-follow my-3 <?php if ($Follow['id'])
                echo 'active'; ?> px-md-5"
                data-id="<?php echo $Listing['id']; ?>">
                <?php echo ($Follow['id'] ? __('Following') : __('Follow')); ?>
            </button>
        <?php } ?>
        <div class="d-flex align-items-center" style="margin-left:15px;">
            <?php if ($AuthUser['id']) { ?>
                <div class="dropdown">
                    <button type="button" class="btn-svg save" data-toggle="modal" data-target="#m"
                        data-remote="<?php echo APP . '/modal/collection?id=' . $Listing['id']; ?>">
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-share" aria-labelledby="shareDropdown" style="width:250px;">
                    <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo post($Listing['id'], $Listing['self'], $Listing['type']); ?>"
                        target="_blank" rel="noopener" style="float:left;background-color: #fff;margin:5px;">
                        <i class="fab fa-facebook" style="font-size:47px;color:#4267B2"></i>
                    </a>
                    <a href="https://twitter.com/intent/tweet?ref_src=twsrc%5Etfw&url=<?php echo post($Listing['id'], $Listing['self'], $Listing['type']); ?>"
                        target="_blank" rel="noopener" style="float:left;background-color:#1DA1F2;margin:5px;">
                        <i class="fab fa-twitter" style="font-size:25px;color:#fff"></i>
                    </a>
                    <a href="https://www.reddit.com/submit?url=<?php echo post($Listing['id'], $Listing['self'], $Listing['type']); ?>"
                        target="_blank" rel="noopener" style="float:left;background-color:#fff;margin:5px;">
            ...
            <?php if ($AuthUser['id']) { ?>
                <button type="button" class="btn-svg report mr-0" data-toggle="modal" data-target="#m"
                    data-remote="<?php echo APP . '/modal/report?id=' . $Listing['id']; ?>">
                    <svg class="icon" stroke-width="3">
                        <use xlink:href="<?php echo ASSETS . '/img/sprite.svg#alert'; ?>" />
                    </svg>
    ...
    <div class="app-detail-embed">
        <div class="embed-col">
            ...
            <div class="embed-play">
                <!-- Removed the release date check -->
                <div class="embed-cover lazy"
                    data-src="<?php echo ($Listing['type'] == 'serie') ? $Listing['episode_image'] : $Listing['cover']; ?>">
                </div>
    ...
    <?php if ($Listing['notification'] !== NULL) { ?>
    ...
    <?php if ($Listing['type'] == 'movie' && $Listing['movie_download'] !== NULL) { ?>
        <a href="<?php echo $Listing['movie_download']; ?>" target=_"blank">
            <div class="alert bg-danger text-white font-weight-bold text-center"
                style="background-color:<?php echo get($Settings, 'data.general', 'theme'); ?>!important">
                Download Movie
            </div>
        </a>
    <?php } ?>

    <?php if ($Listing['type'] == 'serie' && $Listing['episode_download'] !== NULL) { ?>
    ...
    <?php if ($Listing['trailer']) { ?>
        <button type="button" class="btn btn-theme px-5 my-3 mr-2 trailer" style="margin-top: -0px !important;"
            data-toggle="modal" data-target="#lg"
